...
public class Polynki implements Drawing {
    @Override
    public void draw(int width, int height, int level, Graphics2D graphics2D) {
        ...
        Polynki(top, bottomRight, bottomLeft, level, graphics2D);
    }

    private void Polynki(Point2D.Double top, Point2D.Double bottomRight, Point2D.Double bottomLeft, int level, Graphics2D graphics2D) {
        if (level == 0) {
        ...
        Polynki(top, rightThird, leftThird, level - 1, graphics2D);
        Polynki(leftThird, bottomThird, bottomLeft, level - 1, graphics2D);
        Polynki(rightThird, bottomRight, bottomThird, level - 1, graphics2D);
    }
}
